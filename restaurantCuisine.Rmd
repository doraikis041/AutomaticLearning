...
```{r}
 ...
 resultado <- funModeling::df_status(allRestaurant)
```
...
pairs.panels(allRestNum[,2:6], stars = TRUE)
...
```{r Distancia de Gower, warning=FALSE}
dfGowerDist <- daisy(dfAllRestaurant, metric = "gower")

#Generar el histograma dfGowerDist
hist(dfGowerDist, xlim = range(0.0,1.0), main = "Histograma de distancia de Gower")
```
# Fase 4: Modelado
...
### 4.1.1.1 Análisis del Elbow
```{r Evaluación: Diana - Elbow }
ggplot(data = data.frame(t(cstats.table(dfGowerDist, clustModelDiv, 15))), 
...
### 4.1.1.2 Análisis de la Silueta
...
### 4.1.1.3 Poda del arbol
```{r Diana: Cortar el arbol}
groupsDiana <- as.factor(cutree(aggl.clust.c, k=6))
head(groupsDiana)
```
...
### 4.2.1 HCLUST: Definición del metodo
...
```{r}
dfAllRestaurant$clusterDiana <- groupsDiana
dfAllRestaurant$clusterHclust <- groupsHclust
head(dfAllRestaurant)
```
